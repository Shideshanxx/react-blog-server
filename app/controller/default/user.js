...
class UserController extends BaseController {
  ...
  async getUserInfo() {
    ...
    if (
      ...
      this.ctx.request.header.authorization !== "null"
    ) {
    ...
    const sql = `SElECT 
      ...
      ${
        userInfo
          ? `LEFT JOIN fans f ON f.userId = ${id} AND f.fansId = ${userInfo.userId}`
          : ""
      }
      WHERE u.id = '${id}'`;
  ...
  async getMsgList() {
    ...
    let sql = `
      ...
      msg m
      LEFT JOIN user u ON m.userId = u.id
      WHERE ${type ? typeSwitch(type) : ""} m.callUserId = '${
      userInfo.userId
    }' AND m.userId != ${userInfo.userId}
      ORDER BY createTime DESC
